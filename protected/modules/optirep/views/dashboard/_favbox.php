...
$rep_id = Yii::app()->user->id;
  ...
  if ($urole == "admin") {
    ...
    $viewall =  (!empty($myusers))?CHtml::link(Myclass::t('OG038', '', 'og'), array('/optirep/repAccounts'), array('class' => 'topviewall')):'';
  }else{
    $myfavourites = Yii::app()->db->createCommand() //tihs query contains all the data
                    ...
                    ->limit(4)
                    ->queryAll();  
    $viewall =  (!empty($myfavourites))?CHtml::link(Myclass::t('OG038', '', 'og'), array('/optirep/repFavourites'), array('class' => 'topviewall')):'';
  }
      
?>
<div class="col-xs-12 col-sm-6 col-md-4 col-lg-4">
    <div class="cate-bg"> 
        <div class="cate-heading">  <?php echo ($urole == "admin") ? "<i class='fa fa-users'></i> My Reps" : "<i class='fa fa-heart'></i> Favorites"; ?> <?php echo $viewall; ?></div>
        <?php
        if ($urole == "admin") {
            $my_usrs = '';           
            if (!empty($myusers)) {
