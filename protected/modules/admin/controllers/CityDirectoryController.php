...
class CityDirectoryController extends Controller
{
	...
	public function actionCreate()
	{
		$model=new CityDirectory;
                ...
                $data['country'] = CHtml::listData($country, 'ID_PAYS', 'NOM_PAYS_EN');
                
                $criteria_reg = new CDbCriteria;
                $criteria_reg->order = 'NOM_REGION_EN ASC';
                $regions = RegionDirectory::model()->findAll($criteria_reg);
                $data['regions'] = CHtml::listData($regions, 'ID_REGION', 'NOM_REGION_EN');
		...
		if(isset($_POST['CityDirectory']))
		{
			...
			if($model->save()){
                                Yii::app()->user->setFlash('success', 'CityDirectory Created Successfully!!!');
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['CityDirectory']))
		{
			$model->attributes=$_POST['CityDirectory'];
			if($model->save()){
                                Yii::app()->user->setFlash('success', 'CityDirectory Updated Successfully!!!');
                                $this->redirect(array('index'));
                        }
		}
	...
	public function actionDelete($id)
	{
		...
		if(!isset($_GET['ajax'])){
                    Yii::app()->user->setFlash('success', 'CityDirectory Deleted Successfully!!!');
