<tr id="pollchoice-<?php echo $id.'-'.$flag ?>">
  <td class="labeltxt col-sm-5" style="padding-top: .5em; padding-bottom: .5em;">
    ...
    <?php echo CHtml::error($choice,"[$id][]label"); ?>
      <?php }else{?>
      <?php echo CHtml::activeTextField($choice,"[$id][]label_FR",array('class' => 'form-control','size'=>60)); ?>
    <?php echo CHtml::error($choice,"[$id][]label_FR"); ?>
      <?php }?>
    <div class="errorMessage" style="display:none">You must enter a label.</div>
  </td>
  <td class="actions col-sm-2">
  <?php
    $deleteJs = 'jQuery("#pollchoice-fr-'. $flag .'").find("td").fadeOut(1000,function(){jQuery(this).parent().remove();});jQuery("#pollchoice-en-'. $flag .'").find("td").fadeOut(1000,function(){jQuery(this).parent().remove();});return false;';
