...
class P3ArrayValidator extends CValidator {
    ...
    protected function validateAttribute($object,$attribute) {
        ...
        elseif(!is_array($value)) {
            $message=$this->message!==null?$this->message:Myclass::t('{attribute} is not valid.');
            $this->addError($object,$attribute,$message);
        }

        $length=count($value);
        if($this->min!==null && $length<$this->min) {
            $message=$this->tooShort!==null?$this->tooShort:Myclass::t('{attribute} is too short (minimum is {min} entries).');
            $this->addError($object,$attribute,$message,array('{min}'=>$this->min));
        }
        if($this->max!==null && $length>$this->max) {
            $message=$this->tooLong!==null?$this->tooLong:Myclass::t('{attribute} is too long (maximum is {max} entries).');
            $this->addError($object,$attribute,$message,array('{max}'=>$this->max));
        }
        if($this->is!==null && $length!==$this->is) {
            $message=$this->message!==null?$this->message:Myclass::t('{attribute} is of the wrong length (should be {length} entries).');
